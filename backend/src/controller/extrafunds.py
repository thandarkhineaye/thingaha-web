...
@api.route("/extrafunds/<int:extrafunds_id>", methods=["GET"])
@jwt_required
@cross_origin()
def get_extrafunds_by_id(extrafund_id: int):
    ...
    """
    try:
        extrafunds = extrafunds_service.get_extrafunds_by_id(extrafund_id)
...
def create_extrafunds():
    ...
    try:
        current_app.logger.info("create extrafunds")
        extrafund_id = extrafunds_service.create_extrafunds({
...
@api.route("/extrafunds/<int:extrafunds_id>", methods=["PUT"])
@jwt_required
@cross_origin()
def update_extrafunds(extrafund_id: int):
    ...
    try:
        ...
        return jsonify({
            "status": extrafunds_service.update_extrafunds_by_id(extrafund_id, data)
        }), 200
    except (SQLCustomError, ValidateFail, RequestDataEmpty) as error:
...
@api.route("/extrafunds/<int:extrafunds_id>", methods=["DELETE"])
@jwt_required
@cross_origin()
def delete_extrafunds(extrafund_id: int):
    ...
    try:
        ...
        return jsonify({
            "status": extrafunds_service.delete_extrafunds_by_id(extrafund_id)
        }), 200
    except SQLCustomError as error:
